...
public class DisableUserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.disable_user);
    ...
    public void connectClicked(View view) {
        ...
        if(spinner.getSelectedItemId()==0){
            if(value.length()==0){
                ...
                String commandOutput = exec.startConnection(ServerInstance.getInstance(),"usermod -L "+value );
        ...
        if(value.length()==0){
            String commandOutput = exec.startConnection(ServerInstance.getInstance(),"usermod -L "+spinner.getSelectedItem().toString());
            if(commandOutput.length()==0){
                Toast.makeText(getApplicationContext(),"Command Successful",Toast.LENGTH_SHORT).show();
            }
        }
